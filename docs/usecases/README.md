...
Current use cases:
- `01-vscode-portable-project-env.md`: start portable VS Code inside the project Flox environment so the editor, Copilot, Python, and Swift resolve from the repository toolchain, with Python CLI/runtime workflows still using the managed Flox Python venv helper
...
- `03-swift-build-and-test.md`: run Swift build and test workflows through the Flox-managed repository wrapper with artifacts forced under `build/swift`
